...
if (empty($ourphp_weburl[0])){$ourphp_Language = $homelang[0];}else{$ourphp_Language = dowith_sql($ourphp_weburl[0]);}
...
global $db;
$ourphp_rs = $db-> ourphpsql("select `OP_Temppath` from `ourphp_temp` where `id` = 1"); 
return $ourphp_rs[0];
}
...
$smarty->assign('templatepath',$ourphp_webpath.$ourphp_templates."/");
$smarty->assign('listid',$listid);


function ourphp_web(){ 
global $ourphp_webpath,$db,$ourphp_Language,$temptype,$listid,$viewid,$Parameterse,$ourphp_cache;
$ourphp_rs = $db-> ourphpsql("select a.* ,b.OP_Langtitle,b.OP_Langkeywords,b.OP_Langdescription from `ourphp_web` a, `ourphp_lang` b where a.id = 1 && b.OP_Lang = '".$ourphp_Language."'"); 
...
//获取IP等常用参数
function getIP(){
	global $ourphp_Language,$temptype,$listid,$viewid,$ourphp_webpath;
	if (@$_SERVER["HTTP_X_FORWARDED_FOR"]){
		...
		$rows = array(
						...
						'listid' => $listid,
						'viewid' => $viewid,
						'ourphpurl' => $ourphp_webpath.'?'.$_SERVER["QUERY_STRING"],
					);
	return $rows; 
}
