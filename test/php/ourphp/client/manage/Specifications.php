...
$sql="select id,OP_Title,OP_Titleto from `ourphp_productspecifications` where `id` in (".$id.") order by OP_Sorting asc,id asc";
...
$sql="select OP_Value from `ourphp_productspecifications` where `id` in (".$id.") order by OP_Sorting asc,id asc";
$query=mysql_query($sql);
$rows=array();
while($ourphp_rs=mysql_fetch_array($query)){
