...
if(isset($_GET["ourphp_cms"]) == ""){
	echo '';
			...
			`OP_Text` = '".admin_sql($_POST["OP_Text"])."',
			`OP_Sorting` = '".admin_sql($_POST["OP_Sorting"])."',
			`time` = '".date("Y-m-d H:i:s")."'
			";
			$query=mysql_query($sql);
			$ourphp_font = 1;
			$ourphp_class = 'ourphp_productattribute_o.php?id=ourphp&oid='.intval($_GET['oid']);
...
function Attribute(){
	global $conn;
	$sql="select id,OP_Title,OP_Sorting from `ourphp_productattribute` where `OP_Class` = ".intval($_GET['oid'])." order by OP_Sorting asc";
	$query=mysql_query($sql);
	$rows=array();
	$i = 1;
...
if (isset($_GET["pid"])){
$sql="select * from `ourphp_productattribute` where `id` = ".intval($_GET['pid'])."";
$query=mysql_query($sql);
$ourphp_rs=mysql_fetch_array($query);
$smarty->assign('ourphp_productattribute',$ourphp_rs);
mysql_free_result($query);
}
$smarty->display('ourphp_productattribute_o.html');
?>