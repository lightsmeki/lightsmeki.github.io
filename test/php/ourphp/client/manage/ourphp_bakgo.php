...
$db_host=$ourphp_mysqlurl;    //数据库服务器
$db_username=$ourphp_mysqlname;     //数据库用户名
$db_password=$ourphp_mysqlpass;         //数据库密码
$db_dbname=$ourphp_mysqldb;          //选择的数据库
...
function csssetting(){
  return "<style type='text/css'>
	<!--
	body, td, input, a{color:#000000;font-family: '宋体';font-size: 9pt;}
	body, td, a{line-height:180%;}
	.tabletitle{color:#000000;background-color:#ebebeb;margin-top:30px;}
	.tabledata{background-color:#FFffff;}
	.tabledata_on{background-color:#f4f4f4;}	
	
	-->
	</style><link href='templates/images/ourphp_login.css' rel='stylesheet' type='text/css'>
...
function tablestart($ttitle="",$twidth="100%"){
	return "<table width='98%' border='0' cellspacing='0' cellpadding='0' align=center class='tabletitle'>
...
function fbutton($type="submit",$name="Submit",$value="确定",$script="",$return=0){
	$str="<div style='float:left; margin-left:6px;'><input type='$type' name='$name' value='$value' class='ourphp-anniu' $script></div>";
...
if (!isset($_POST['action'])){
	...
	$query=mysql_query("SHOW TABLE STATUS LIKE 'ourphp%'"); //ourphp开头的
...
if($_POST['action']=="selectoption"){
	if($_POST['back_type']=="partsave"){//多文件保存选项
		...
		echo tabledata("存放目录：|../../function/backup/".date('Ymd')."/|相对本程序所在目录，必须有写入权限<input type='hidden' name='dir' value='../../function/backup/".date('Ymd')."' /><input name='filename' value='index' type='hidden' class='win2'>");
		...
		echo tabledata("确认导入密码：|<input name='back_pass2' value='' type='password' class='win'>|不添默认为ourphp.net");
		echo tableend();
		echo "
		<script language='JavaScript'>
		function confirmit(){
...
if($_POST['action']=="databackup"){
	...
	function mysql_functions(){
		...
		function query($sql){
			global $_POST;
			if(!mysql_query($sql)){
				echo "<BR><BR><font color=red>MySQL语句错误！您可能发现了程序的BUG！<a href=\"ourphp.net\" target=\"_blank\">请报告开发者。</a>
...
$writefile_data .= "$data";
writefile($writefile_data,"w");
$backpassword=$_POST[back_pass]?$_POST[back_pass]:"ourphp.net";
		//引导文件内容
$data='<?php
...
tableend().
tablestart('导入数据库配置').
tabledata('服务器：|<input name="db_host" value="<?php echo $ourphp_mysqlurl ?>" type="text">',"50%|50%").
tabledata('数据库：|<input name="db_dbname" value="<?php echo $ourphp_mysqldb ?>" type="text">').
tabledata('该数据库不存在时自动创建|<input name="db_autocreate" value="1" type="checkbox">').
tabledata('用户名：|<input name="db_username" value="<?php echo $ourphp_mysqlname ?>" type="text">').
tabledata('密　码：|<input name="db_password" value="<?php echo $ourphp_mysqlpass ?>" type="password">').
...
<div id="pageendTag"></div>
		...
		<BR><BR><a href='$_POST[dir]/{$_POST[filename]}.php' target='_blank'><H3>运行备份文件 {$_POST[filename]}.php </H3></a> 也可将数据恢复。
		<BR><BR>",500);
